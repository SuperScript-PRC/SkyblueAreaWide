...
# 电池供能
execute as @e[tag=st.dc,scores={st:mc_rf=100..}] at @s if score @e[c=1,r=15,scores={st:mc_mcp=1..}] st:mc_rf < @e[c=1,r=15,scores={st:mc_mcp=1..}] st:mc_srf run tag @s add st.dc_active
execute as @e[tag=st.dc_active] at @s run scoreboard players remove @s st:mc_rf 100
execute as @e[tag=st.dc_active] at @s run scoreboard players add @e[c=1,r=15,scores={st:mc_mcp=1..}] st:mc_rf 100
# #(循环)#{40}
# 太阳能电池板控制
